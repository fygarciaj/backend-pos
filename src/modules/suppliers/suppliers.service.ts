...
export class SuppliersService {
  ...
  async create(createSupplierDto: CreateSupplierDto): Promise<Supplier> {
    ...
    const existingName = await this.prisma.supplier.findUnique({ where: { name } });
    if (existingName) {
      throw new ConflictException(`Supplier with name "${name}" already exists.`);
    }
    // Verificar si el email ya existe (si se proporciona)
    if (email) {
        const existingEmail = await this.prisma.supplier.findFirst({ where: { email } });
         if (existingEmail) {
    ...
    try {
      ...
      if (brandIds && brandIds.length > 0) {
          const brandsCount = await this.prisma.brand.count({ where: { id: { in: brandIds } } });
          if (brandsCount !== brandIds.length) {
              throw new NotFoundException('One or more provided Brand IDs do not exist.');
          }
      }
  ...
  async findAll(params: {
    ...
    return this.prisma.supplier.findMany({
      ...
      include: {
          _count: includeRelations ? { select: { purchaseOrders: true, brands: true } } : undefined,
          brands: includeRelations ? { select: { brand: { select: { id: true, name: true }}}} : false,
          // purchaseOrders: includeRelations ? { take: 5, orderBy: { orderDate: 'desc' } } : false, // Podría ser pesado
      },
    });
  }

  async findOne(id: string, includeRelations: boolean = true): Promise<Supplier | null> {
    const supplier = await this.prisma.supplier.findUnique({
      where: { id },
      include: {
          _count: includeRelations ? { select: { purchaseOrders: true, brands: true } } : undefined,
          brands: includeRelations ? { include: { brand: true } } : false, // Incluir detalles de marca
          purchaseOrders: includeRelations ? { take: 10, orderBy: { orderDate: 'desc' } } : false, // Incluir últimas órdenes
      },
    });
    if (!supplier) {
  ...
  async update(id: string, updateSupplierDto: UpdateSupplierDto): Promise<Supplier> {
    ...
    if (name) {
        const existingName = await this.prisma.supplier.findFirst({ where: { name, id: { not: id } } });
        if (existingName) {
            throw new ConflictException(`Supplier with name "${name}" already exists.`);
        }
    ...
    if (email) {
        const existingEmail = await this.prisma.supplier.findFirst({ where: { email, id: { not: id } } });
        if (existingEmail) {
            throw new ConflictException(`Supplier with email "${email}" already exists.`);
        }
        dataToUpdate.email = email;
    } else if (email === null) { // Permitir borrar email
    ...
    if (brandIds !== undefined) {
        ...
        if (brandIds.length > 0) {
            const brandsCount = await this.prisma.brand.count({ where: { id: { in: brandIds } } });
            if (brandsCount !== brandIds.length) {
                throw new NotFoundException('One or more provided Brand IDs do not exist for update.');
            }
        }
  ...
  async remove(id: string): Promise<Supplier> {
    ...
    if (supplierToDelete._count && supplierToDelete._count.purchaseOrders > 0) {
      throw new ConflictException(`Cannot delete supplier "${supplierToDelete.name}" because they have ${supplierToDelete._count.purchaseOrders} associated purchase orders. Please reassign or delete them first.`);
    }

    // Si no hay dependencias, proceder a borrar
    // Prisma manejará el borrado en cascada de SupplierBrand
    try {
      this.logger.log(`Deleting supplier: ${supplierToDelete.name} (ID: ${id})`);
  ...
  private handleDbError(error: any, context?: any): never {
    if (error instanceof Prisma.PrismaClientKnownRequestError) {
      if (error.code === 'P2002') { // Unique constraint violation
        ...
        if (fields?.includes('name') && context?.name) message = `Supplier with name "${context.name}" already exists.`;
        else if (fields?.includes('email') && context?.email) message = `Supplier with email "${context.email}" already exists.`;
