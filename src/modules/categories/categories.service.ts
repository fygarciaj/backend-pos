...
export class CategoriesService {
  ...
  async create(createCategoryDto: CreateCategoryDto): Promise<Category> {
    ...
    if (parentCategoryId) {
      const parentExists = await this.prisma.category.findUnique({ where: { id: parentCategoryId } });
      if (!parentExists) {
  ...
  async findAll(params: {
    ...
    const { skip, take, cursor, where, orderBy, includeProductsCount, includeChildren } = params;
    return this.prisma.category.findMany({
      ...
      include: {
        ...
        childCategories: includeChildren ? { include: { _count: { select: { products: true }}}} : false, // Incluir hijos si se solicita
  ...
  async update(id: string, updateCategoryDto: UpdateCategoryDto): Promise<Category> {
    ...
    if (name || slug) {
        ...
        const finalSlug = slug ? slugify(slug) : (name ? slugify(name) : currentCategory.slug);
        ...
        if (existingCategory) {
            if (existingCategory.name === finalName) {
                throw new ConflictException(`Category with name "${finalName}" already exists.`);
            }
            if (existingCategory.slug === finalSlug) {
                throw new ConflictException(`Category with slug "${finalSlug}" already exists.`);
            }
    ...
    if (parentCategoryId !== undefined) { // Permite establecer parent a null
  ...
  async remove(id: string): Promise<Category> {
    ...
    const hasChildren = await this.prisma.category.count({ where: { parentCategoryId: id } });

    if (hasProducts > 0) {
      throw new ConflictException(`Cannot delete category "${categoryToDelete.name}" because it has ${hasProducts} associated products. Please reassign or delete them first.`);
    }
    if (hasChildren > 0) {
      throw new ConflictException(`Cannot delete category "${categoryToDelete.name}" because it has ${hasChildren} subcategories. Please reassign or delete them first.`);
    }
  ...
  private handleDbError(error: any, context?: any): never {
    if (error instanceof Prisma.PrismaClientKnownRequestError) {
      if (error.code === 'P2002') { // Unique constraint violation
        ...
        if (fields?.includes('name') && context?.name) message = `Category with name "${context.name}" already exists.`;
        else if (fields?.includes('slug') && context?.slug) message = `Category with slug "${context.slug}" already exists.`;
        throw new ConflictException(message);
      }
       if (error.code === 'P2025') { // Record not found (e.g., during update/delete)
